...
//Counts the number of these bids for pagination
	...
	$num_recco_result = $mysqli->query($num_recco_query)
...
<?php
	...
	$result = $mysqli->query($recco_listing_query)
	...
	while ($row = mysqli_fetch_array($result))
	{
		...
		if ($bid_count[0] == 0) {
			...
			$top_bid_query = "SELECT MAX(biding_price) FROM biding WHERE listing_id = {$row['listing_id']}";

			$top_bid_result = $mysqli->query($top_bid_query)
				or die('Error making top bid query');

			$top_bid = mysqli_fetch_array($top_bid_result);

			print_listing_li($row['listing_id'], $row['title'], $row['details'], $top_bid[0], $bid_count[0], date_create($row['endtime']));

		}*/
		
		
		$getBidingNumQuery = "SELECT COUNT(*) from biding WHERE listing_id = {$row['listing_id']}";
