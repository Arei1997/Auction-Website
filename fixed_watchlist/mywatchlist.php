...
#}
	...
	$num_watchlist_result = $mysqli->query($num_watchlist_query)
...
<?php
	...
	$result = $mysqli->query($watchlist_listing_query)
	...
	while ($row = mysqli_fetch_array($result))
	{
		...
		$count_bid_result = $mysqli->query($count_bid_query)
			or die('Error making count bid query');
		...
		if ($bid_count[0] == 0) {
			...
			print_listing_li($row['listing_id'], $row['title'], $row['details'], $top_bid[0], $bid_count[0], date_create($row['end_date']));
